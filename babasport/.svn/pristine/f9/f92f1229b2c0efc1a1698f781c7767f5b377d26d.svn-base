				...
				<tbody class="pn-ltbody">
					<c:forEach items="${pagination.list }" var="entry">
						...
						<tr bgcolor="#ffffff" onmouseout="this.bgColor='#ffffff'"
							...
							<td align="center"><a class="pn-opt"  onclick="window.location.href='/brand/toEdir.do?id=${entry.id }'" href="javascript:void(0)">修改</a> | <a
								class="pn-opt"
								onclick="if(!confirm('您确定删除吗？')) {return false;} 
							window.location.href='/brand/delete.do?id=${entry.id }&name=${name}&isDisplay=${isDisplay}'"
